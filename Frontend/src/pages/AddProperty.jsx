...
const AddProperty = () => {
  ...
  useEffect(() => {
    ...
    if (parsedUser.role === 'tenant') {
      toast.error('Only landlords can add properties. Please contact admin to change your role.');
      navigate('/search');
    }
  }, [navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!token) {
        toast.error('Please login again to add a property');
        navigate('/login');
        return;
      }

      let posterImageUrl = 'https://images.unsplash.com/photo-1560448204-e02f11c3d0e2?w=400&h=300&fit=crop&crop=center';
      
      // If user uploaded images, convert first image to base64 data URL for now
      ...
      const response = await fetch('https://roomsewa-project-production.up.railway.app/api/rooms/', {
        method: 'POST',
        headers: {
      ...
      toast.success('Property submitted for review! It will be listed after admin approval.');
            ...
            <div className="flex items-center space-x-4">
              ...
              <Link to="/" className="flex items-center hover:scale-105 transition-transform duration-200">
                ...
                <div className="ml-3">
                  ...
                  <p className="text-xs text-gray-500">Add Your Property</p>
                </div>
              </Link>
            </div>

            {/* User info */}
            <div className="flex items-center space-x-3 bg-cyan-50/90 backdrop-blur-sm border border-cyan-200 rounded-xl px-4 py-2 hover:bg-cyan-100 transition-all duration-200">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
          <div className="bg-gradient-to-r from-cyan-600 to-cyan-700 px-8 py-6">
            ...
            <p className="text-cyan-100 mt-2">Fill in the details to list your property</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-8">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {availableAmenities.map(amenity => (
                  ...
                  >
                    <FontAwesomeIcon icon={amenity.icon} className="text-lg" />
                    <span className="text-sm font-medium">{amenity.label}</span>
                    {formData.amenities.includes(amenity.value) && (
                      <FontAwesomeIcon icon={faCheck} className="text-cyan-500 ml-auto" />
                    )}
                  </button>
                ))}
              </div>
            ...
            <div className="flex justify-end space-x-4 pt-6 border-t border-gray-200">
              ...
              <button
                type="submit"
                disabled={uploading}
                className="px-8 py-3 bg-gradient-to-r from-cyan-600 to-cyan-700 hover:from-cyan-700 hover:to-cyan-800 text-white font-semibold rounded-xl transition-all duration-300 hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
              >
